    ...
    <div class="row justify-content-center">
      <div class="col-sm-6 col-xl-4 pb-3">
        <div class="card text-black">
          <div class="card-header h5 text-white text-center bg-gradient-blue">Subscription</div>
          <div class="card-body p-md-5 mx-md-4">
            <form method="POST"
                  class="needs-validation"
                  novalidate>
              @csrf
              <div class="note note-secondary fs-5 mb-4 fst-italic">
                You will receive timely incidents & breaking news alerts sent to your email address.
              </div>
              <div class="text-muted fst-italic">
                <i class="fas fa-arrow-right"></i> Total due now:
                <span class="text-danger fw-bold">${{ $price . '.00' }}</span>
              </div>
              <div class="text-muted fst-italic">
                <i class="fas fa-arrow-right"></i> Your subscription will start from: <span
                  class="text-danger fw-bold">{{ date('M d Y') }}</span>
              </div>
                  ...
                  <div class="creditcard">
                    ...
                    <g id="Front">
                      ...
                      <text transform="matrix(1 0 0 1 60.106 295.0121)" id="svgnumber"
                            class="st2 st3 st4">{{ '**** **** **** ' . $last4 }}
                      </text>
                      <text transform="matrix(1 0 0 1 54.1064 428.1723)" id="svgname"
                            class="st2 st5 st6">{{ str($cardHolderName)->upper() }}
                      </text>
                      ...
                      <g>
                        <text transform="matrix(1 0 0 1 574.4219 433.8095)" id="svgexpire"
                              class="st2 st5 st9">{{ $exp_month < 10 ? '0' . $exp_month : $exp_month . '/' . str($exp_year)->substr(2) }}
                        </text>
                    ...
                    C707.1,246.4,704.4,249.1,701.1,249.1z"/>
                          ...
                          <text transform="matrix(1 0 0 1 59.5073 228.6099)" id="svgnameback"
                                class="st12 st13">{{ $cardHolderName }}
                          </text>
                        </g>
                </svg>
                    </div>
              ...
              <div class="d-flex justify-content-center">
                <a href="{{ route('stripe.add-payment-method') }}" class="btn btn-outline-primary">
                  + Change Payment Method
                </a>
              </div>

              <div class="card my-4 d-none">
                <div class="card-body text-muted p-0">
                  <div class="card-text fw-bold fs-4 ps-4 mt-4">{{ $cardHolderName }}</div>
                  <div class="ms-4"><img width="70px" src="{{ asset('img/card/chip.png') }}" alt="card logo"></div>
                  <div class="">
                    @foreach(range(1, 12) as $index => $i)
                      @if($index % 4 == 0)
                        &nbsp &nbsp &nbsp
                      @endif
                      <i class="fa-solid fa-circle"></i>
                    @endforeach
                    <span class="fs-4 fw-bold">&nbsp&nbsp{{ $last4 }}</span>
                  </div>
                  <div class="d-flex justify-content-between align-items-center">
                    <div class="fw-bold fs-4 ps-4">
                      {{ $exp_month < 10 ? '0' . $exp_month : $exp_month }}/{{ str($exp_year)->substr(2) }}
                    </div>
                    <div class="pe-4">
                      <img width="70px" src="{{ asset('img/card/' . $brand  . '.png') }}" alt="card logo">
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="form-check form-check-inline mt-5 mb-4">
                <input class="form-check-input" type="checkbox" value="1"
                       name="tou"
                       id="tou" required
                       @if(old('tou') == 1) checked @endif/>
                <label class="form-check-label d-inline" for="tou">
                  I have read and agree with the <a
                    href="https://911breakingnews.com/terms-of-use-service" target="_blank">911
                    Breaking News Terms
                    of Use <span class="text-red fw-bold">*</span></a>
                </label>
              </div>
              <button type="submit" class="btn btn-primary btn-block bg-gradient-blue fw-bold mb-4">
                Confirm & Pay
              </button>
            </form>
          </div>
